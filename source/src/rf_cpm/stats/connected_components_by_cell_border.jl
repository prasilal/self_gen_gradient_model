...
"""
function connected_components_by_cell_border(model, cellid)
    cbp = get_stat(model, :border_pixels_by_cell)
...
""" Creates an object with connected components of the border of each cell on the grid.
    ...
    (specified by {@link ArrayCoordinate}) belonging to that cell.
"""
function connected_components_by_cell_border_stat(model)
    components = Dict()

    for ci in cell_ids(model)
        components[ci] = connected_components_by_cell_border(model,  ci)
    end
