...
function get_cfg()
    cfg = Dict(
        ...
        :rule_graph => @d(
            ...
            :zg => @d(
                :wall => [],
                :e => [(:e, :prod_v, 1000),(:e, :adhesion, 100), (:e, :volume, 50, 30),
                        (:e, :perimeter, 2, 70)],
...
for y in [1000,5000,10000]
    ...
    for i in 1:10
        seed = rand(1:9999)
        seed, time = sim_test(true,seed,production)
        push!(df_sim,[seed,true,time,production])
        println("Simulace číslo "*string(i)*"-true proběhla úspěšně")
        println("počet kroků: "*string(time))
        seed, time = sim_test(false,seed,production)
        push!(df_sim,[seed,false,time,production])
        println("Simulace číslo "*string(i)*"-false proběhla úspěšně")
        println("počet kroků: "*string(time))
    end
end
CSV.write("data.csv", df_sim, append=true, bufsize=2^28)
